...
def test_wrap_jax_simple_real():
    ...
    im_orig = galsim.Image([[ 11., 12., 13., 14., 11., 12., 13., 14. ],  # noqa: E201,E202
                            [ 21., 22., 23., 24., 21., 22., 23., 24. ],  # noqa: E201,E202
                            [ 31., 32., 33., 34., 31., 32., 33., 34. ],  # noqa: E201,E202
                            [ 41., 42., 43., 44., 41., 42., 43., 44. ],  # noqa: E201,E202
                            [ 11., 12., 13., 14., 11., 12., 13., 14. ],  # noqa: E201,E202
                            [ 21., 22., 23., 24., 21., 22., 23., 24. ],  # noqa: E201,E202
                            [ 31., 32., 33., 34., 31., 32., 33., 34. ],  # noqa: E201,E202
                            [ 41., 42., 43., 44., 41., 42., 43., 44. ]])  # noqa: E201,E202
    im = im_orig.copy()
    ...
    im_quad = im_orig[b]
    im_wrap = im.wrap(b)
    np.testing.assert_allclose(im_wrap.array, 4. * im_quad.array)
    ...
    for xmin, ymin in ((1, 5), (5, 1), (5, 5), (2, 3), (4, 1)):
        ...
        np.testing.assert_allclose(im_wrap.array, im[b].array,
...
def test_wrap_jax_weird_real():
    ...
    im_wrap = im.wrap(b)
    np.testing.assert_allclose(im_wrap.array, im_test.array,
                               err_msg="image.wrap(%s) did not match expectation" % b)
...
def test_wrap_jax_complex():
    ...
    for i in range(-M, M + 1):
        for j in range(-N, N + 1):
            ...
            val = np.exp((i/(2.3*M))**2 + 1j*(2.8*i-1.3*j)) + ((2 + 3j*j)/(1.9*N))**3  # noqa: E226
            # val = 2*(i-j)**2 + 3j*(i+j)
    ...
    np.testing.assert_allclose(im2_wrap.array, im_test[b2].array,
                               err_msg="image.wrap(%s) did not match expectation" % b)
    ...
    np.testing.assert_allclose(im3_wrap.array, im_test[b3].array,
                               err_msg="image.wrap(%s) did not match expectation" % b)
