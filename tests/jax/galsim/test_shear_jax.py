...
q = [1.0, 0.5, 0.3, 0.1, 0.7, 0.9, 0.99, 1.0 - 8.75e-5]
n_shear = len(q)
beta = [
    0.0,
    0.5 * np.pi,
...
def all_shear_vals(test_shear, index, mult_val=1.0):
    ...
    test_vec = [
        np.abs(mult_val) * g[index],
        mult_val * g1[index],
        mult_val * g2[index],
        np.abs(mult_val) * e[index],
        mult_val * e1[index],
        mult_val * e2[index],
        np.abs(mult_val) * eta[index],
        mult_val * eta1[index],
        mult_val * eta2[index],
        mult_val * mult_val * e[index] * e[index],
        q[index],
        test_beta % np.pi,
    ]
    np.testing.assert_array_almost_equal(
        vec, test_vec, decimal=decimal, err_msg="Incorrectly initialized Shear"
    )
    if index == n_shear - 1:
        # On the last one with values < 1.e-4, multiply everything by 1.e4 and check again.
        vec = [1.0e4 * v for v in vec[:-2]]  # don't include q or beta now.
        test_vec = [1.0e4 * v for v in test_vec[:-2]]
        np.testing.assert_array_almost_equal(
            vec, test_vec, decimal=decimal, err_msg="Incorrectly initialized Shear"
        )
...
def test_shear_initialization():
    ...
    assert_raises(TypeError, galsim.Shear, q=0.1, beta=0.0)
...
def test_shear_matrix():
    ...
    for ind in range(n_shear):
        ...
        np.testing.assert_array_almost_equal(
            m1, true_m1, decimal=12, err_msg="getMatrix returned wrong matrix"
        )
