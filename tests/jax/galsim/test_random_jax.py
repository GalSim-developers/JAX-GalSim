...
def test_uniform():
    ...
    np.testing.assert_almost_equal(
        mean, mu, 1, err_msg="Wrong mean from UniformDeviate"
    )
    np.testing.assert_almost_equal(
        var, v, 1, err_msg="Wrong variance from UniformDeviate"
    )
    ...
    # NOTE: these tests differ since we cannot edit arrays in-place in JAX
    # Test generate
    u.seed(testseed)
