...
def _run_spergel_bench_init():
    return jgs.Spergel(nu=jnp.array(-0.6), half_light_radius=3.4).scale_radius
...
@jax.jit
def _run_spergel_bench_init_float():
    return jgs.Spergel(nu=-0.6, half_light_radius=3.4).scale_radius


@pytest.mark.parametrize("kind", ["run"])
def test_benchmark_spergel_init_float(benchmark, kind):
    dt = _run_benchmarks(
        benchmark, kind, lambda: _run_spergel_bench_init_float().block_until_ready()
    )
    print(f"time: {dt:0.4g} ms", end=" ")
