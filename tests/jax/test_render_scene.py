...
def _add_to_image(carry, x, slen):
    ...
    if b.isDefined():
        ...
        start_inds = (i1, j1)
        subim = jax.lax.dynamic_slice(
            image.array, start_inds, (slen, slen)
        )
        subim = subim + stamp.array

        image._array = jax.lax.dynamic_update_slice(
            image.array, subim, start_inds,
        )
...
def _render_scene_stamps_galsim(
    galaxy_params: dict,
    image_pos: list[galsim.PositionD],
    local_wcs: list[galsim.PixelScale],
    fft_size: int,
    slen: int,
    image: galsim.ImageD,
    ng: int,
    ...
    for i in range(ng):
        gpars = {
            k: v[i] for k, v in galaxy_params.items()
        }
        convolved_object = _get_bd_jgs(**gpars).withGSParams(gsparams).to_galsim()

        stamp = convolved_object.drawImage(
            nx=slen, ny=slen, center=image_pos[i], wcs=local_wcs[i], dtype=np.float64
        )
...
def test_render_scene_stamps():
    ...
    stamps = jax.jit(jax.vmap(partial(_draw_stamp_jgs, slen=52, fft_size=256)))(
        galaxy_params, image_positions, local_wcss
    )
    assert stamps.array.shape == (ng, 52, 52)
    assert stamps.array.sum() > 0

    pad_image = jgs.ImageD(
        jnp.pad(image.array, 52), wcs=image.wcs, bounds=image.bounds.withBorder(52)
    )

    final_pad_image = jax.lax.scan(partial(_add_to_image, slen=52), (pad_image,), xs=stamps, length=ng)[0][0]
    ...
    gs_image = jgs.Image(ncol=200, nrow=200, scale=0.2, dtype=np.float64)
    wcs = image.wcs

    gs_image_positions = map(lambda tup: galsim.PositionD(x=tup[0], y=tup[1]), zip(x, y))
    gs_local_wcss = map(lambda x: wcs.local(image_pos=x), gs_image_positions)
