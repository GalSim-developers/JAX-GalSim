...
A Gaussian galaxy convolved with a Gaussian PSF, drawn and noised — equivalent to GalSim's
``demo1.py``.
...
Most GalSim code translates directly by replacing ``import galsim`` with
``import jax_galsim``.
...
.. note::
   ...
   Any arguments that affect control flow (like image size) must be marked as
   ``static_argnames`` for JIT to work.
