...
def calculate_n_photons(
    ...
    Returns:
        ...
        g:              The gain to use when shooting the photons.
    """
    n_photons, g, _ = _calculate_n_photons(
        flux,
        eta_factor,
        max_sb,
        rng,
    ...
    return np.atleast_1d(n_photons).ravel()[0], np.atleast_1d(g).ravel()[0]


@jax.jit
def get_n_photons(n_photons_data):
    ...
    return _n_photons, g, n_photons_data.rng
...
def _sample_zero(n_photons_data):
    Ntot, g, _rng = _calculate_n_photons(
        n_photons_data.flux,
        n_photons_data.flux_per_photon,
        n_photons_data.max_sb,
        rng=n_photons_data.rng,
        max_extra_noise=n_photons_data.max_extra_noise,
        poisson_flux=n_photons_data.poisson_flux,
    )
    return Ntot, g, _rng


@jax.jit
def _calculate_n_photons(
    flux,
    eta_factor,
    max_sb,
    rng,
