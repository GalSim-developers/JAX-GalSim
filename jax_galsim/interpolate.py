    ...
    def __init__(self, x, y, k=3, endpoints="not-a-knot", coefficients=None):
        ...
        if coefficients is None:
            ...
            if k == 3:
                ...
                zero = jnp.array([0.0])
                one = jnp.array([1.0])
                A00 = one if endpoints == "natural" else jnp.array([h[1]])
                A01 = zero if endpoints == "natural" else jnp.array([-(h[0] + h[1])])
                A02 = zero if endpoints == "natural" else jnp.array([h[0]])
                ANN = one if endpoints == "natural" else jnp.array([h[-2]])
                AN1 = (
